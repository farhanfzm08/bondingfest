...
export interface TeamOrPart { id: string; name: string; section?: string|null; groupName?: string|null; }
...
export default function TabBracket({ comp }: { comp: Competition }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (isTeam) {
      ...
      setTeams((rData.individuals||[]).map((r:any)=>({ id:r.participant.id, name:r.participant.name, section:r.participant.section })));
    }
    setLoading(false);
  }, [comp.id, comp.type]);
