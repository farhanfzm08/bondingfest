...
export default function TimeTrialView({ comp, matches, teams, onRefresh }: { comp:Competition; matches:Match[]; teams:TeamOrPart[]; onRefresh:()=>void }) {
  ...
  const ranked = [...entries].sort((a,b)=>{
    const aScore = a.score??-Infinity; const bScore = b.score??-Infinity;
  ...
  const handleAddEntries = async () => {
    ...
    const names = selectedTeams.map(id=>teams.find(t=>t.id===id)?.name||"?").join(", ");
  ...
  for(const m of matches) for(const p of m.participants) { const id=p.teamId||p.participantId||""; if(id) registeredIds.add(id); }
  const unregistered = teams.filter(t=>!registeredIds.has(t.id));
              ...
              {ranked.map((e,i)=>{
                ...
                return (
                  <tr key={e.partId} className={`border-b-2 border-[#E7E5E4] hover:bg-[#FFFBEB] ${i<3?"font-black":""}`}>
                    <td className="px-4 py-3 text-center">{medal||<span className="text-black font-bold text-sm">#{i+1}</span>}{medal&&<span className="text-lg">{medal}</span>}</td>
